...
export default function Layout({ children }) {
  ...
  const msMove = useCallback(
    (e) => {
      if (dragging) {
        let fx = e.clientX - mouseOffset.x + dtMenuSize.w / 2;
        if (fx <= dtMenuSize.w / 2) fx = dtMenuSize.w / 2;
        if (fx >= ws.w - dtMenuSize.w / 2) fx = ws.w - dtMenuSize.w / 2;
        let fy = e.clientY - mouseOffset.y + dtMenuSize.h / 2;
  ...
  const thMove = useCallback(
    (e) => {
      if (dragging) {
        let fx = e.touches[0].clientX - mouseOffset.x + dtMenuSize.w / 2;
        console.log(e.touches);
        if (fx <= dtMenuSize.w / 2) fx = dtMenuSize.w / 2;
        if (fx >= ws.w - dtMenuSize.w / 2) fx = ws.w - dtMenuSize.w / 2;
        let fy = e.touches[0].clientY - mouseOffset.y + dtMenuSize.h / 2;
        if (fy <= dtMenuSize.h / 2) fy = 0 + dtMenuSize.h / 2;
        if (fy >= ws.h - dtMenuSize.h / 2) fy = ws.h - dtMenuSize.h / 2;
        setPos({ x: `${fx}px`, y: `${fy}px` });
      }
  ...
  useEffect(() => {
    if (dragging) {
      ...
      window.addEventListener("touchmove", thMove);
    ...
    <>
      <header>
        <Box
          ...
          top={pos.y}
          left={pos.x}
          pointerEvents={dragging ? `unset` : `auto`}
        >
          <Box w={`100%`} h={`40px`} bg={bar} onMouseDown={msDown}></Box>
          <Flex
